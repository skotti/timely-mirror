...
const PARAM: usize = 2; // <-- should be `2`
const PARAM_OUTPUT: usize = 8; // <-- should be `2` but causes heap corruption if not `8`
...
fn fpga_communication(hc: *const HardwareCommon, h_mem_ptr: *mut u64, o_mem_ptr: *mut u64) {
    ...
    let output_arr2 = unsafe { std::slice::from_raw_parts_mut(o_mem_ptr, 144) };
    let data: &[u64] = &input_arr[FRONTIER_LENGTH..FRONTIER_LENGTH + 16];
    let frontiers: &[u64] = &input_arr[1..1 + 16];
    let output_arr: &mut [u64] = &mut [0; 32];
    ...
    dbg!();
    // assert_eq!(output_arr, &mut output_arr2[0..32]);
}

/// Sends data to FPGA
fn send_input(hc: *const HardwareCommon, mut input: [u64; MAX_LENGTH_IN]) {
    ...
    print!("o_mem: ");
    let output_arr = unsafe { std::slice::from_raw_parts_mut(o_mem_ptr, MAX_LENGTH_OUT) };
    print_array(output_arr);
    ...
    dbg!();
    // Mark as valid
    unsafe { *o_mem_ptr.offset(TR_SIZE_OUT / 8 - 1) = 0 };
    ...
    fn fpga_wrapper(&self, hc: *const HardwareCommon) -> Stream<S, u64> {
        ...
        let raw_logic = move |progress: &mut SharedProgress<S::Timestamp>| {
            ...
            if !has_data {
                ...
                let mut input_memory: [u64; MAX_LENGTH_IN] = [0; MAX_LENGTH_IN];
                inputs += 1;
                input_memory[current_length] = u64::MAX;
                current_length += 1;
